 ...
 * @param {any} - O componente `ProtectedRoute` recebe um `children` como parâmetro. O hook `useAuth`
 ...
 * @param {any} - O componente `RouteIsAuth` é um componente React customizado que recebe um `children`
 ...
 */
export const RouteIsAuth = ({ children }: any) => {
  const { user }: any = useAuth();
  if (user) {
