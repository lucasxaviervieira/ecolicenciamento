...
export default function Agenda() {
  const [tasks, setTasks] = useState<Tasks[]>(LoadingTasks);
  ...
  const addTask =
    (texto: any) => async (event: React.FormEvent<HTMLFormElement>) => {
      loadingComponent();
  ...
  const handleChange = (task: Tasks) => async () => {
    loadingComponent();
  ...
  function verifyIDLicense(currentURL: boolean) {
    return currentURL ? "" : <Navigate to="NOT_FOUND" />;
  }

  async function addTaskApi(jsonParam: any) {
    await api.post("/todo_list/insertTodo_list.php", jsonParam).catch((err) => {
      console.error("Catch Error:" + err);
    });
  }

  async function checkedTaskApi(jsonParam: any) {
  ...
  async function deleteTaskApi(taskId: any) {
  ...
  */
  useEffect(() => {
    async function getData(idParam: string) {
      !idParam
        ...
        : await api
            ...
            .then((response) => {
              ...
              if (haveMessage) {
                ...
                const withoutTasks =
                  response.data?.mensagem == "Nenhum registro encontrado";
                if (withoutTasks) {
    ...
    <>
      ...
      >
        ...
        <div className="w-[60%] flex items-center gap-3 py-2">
          <input
            className="block w-[100%] rounded-md border-0 py-1.5 pl-5 pr-25 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
            value={text}
            placeholder="Ex.: Realizar algo (26/02)"
