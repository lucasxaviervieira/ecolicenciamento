...
import { License, LoadingLicense } from "../../../../utils/licenseType";
...
export default function DashboardGestor() {
  const [data, setData] = useState<License[]>(LoadingLicense);
   ...
   */
  function filterRequisition(responseData: any[]) {
    const dataFormatted: any[] | ((prevState: License[]) => License[]) = [];
    responseData.forEach((element) => {
      if (
        element.situacao_licenca == "Aguardando análise" ||
  ...
  */
  ...
  return (
    <>
      ...
      />
      <div
        className="hidden p-5 h-[87%] flex flex-col justify-center items-center"
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => {
                    return (
                      <th
                        className="sticky top-0 px-2 py-3 z-10 bg-red-900 text-white text-xs lg:text-sm xl:text-lg font-bold"
                        key={header.id}
            ...
            <tbody>
              {table.getRowModel().rows.map((row) => {
                return (
                  <tr
                    key={row.id}
                    className="odd:bg-white even:bg-slate-50 max-w-56 max-h-20 text-xs lg:text-sm xl:text-base "
                  >
                    {row.getVisibleCells().map((cell) => {
                      ...
                      const isPaintable =
                        columnsToPaint.indexOf(idContext) >= 0 ? true : false;
                      ...
                      return (
                        <td className={classNameText} key={cell.id}>
                          {flexRender(
